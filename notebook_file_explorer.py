    ...
    def open_file(self, event=None):
        ...
        if self.tree.item(cur_item)["values"][1] == 'directory':
            ...
            if self.open_file_event is not None:
                self.open_file_event(file_path=file_path)
            else:
        ...
        for file in os.listdir():
            ...
            if os.path.isdir(file):
                for sub in os.listdir(file):
                    file_sub = file + "/" + sub
                    self.tree.insert(file, "end", file_sub, values=(file_sub, "file", self.get_file_size(file_sub)),
                                     text=file_sub)

    def get_file_size(self, file_path):
        return os.path.getsize(file_path)

    def register_to_on_open(self, callback):
        self.open_file_event = callback
