...
from .button_functionality import create_slide_menu, save_result, create_file_explorer,  \
...
from .memento_undo import Originator, Caretaker
...
class Application(tk.Frame):
    ...
    def key(self, event):
        if event.keysym == 'z' and event.state == 4:
            ...
            self.originator.setState(self.instructions_text.get(1.0, tk.END))
            self.caretaker.add(self.originator.getState())
            self.index += 1

    def undo(self):
        if self.index > 0:
            self.index -= 1
            self.originator.getStateFromMemento(self.caretaker.get(self.index - 1))
            self.instructions_text.delete(1.0, tk.END)
            self.instructions_text.insert(1.0,self.originator.getState())
