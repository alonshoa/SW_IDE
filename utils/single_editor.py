...
class SingleEditor(tk.Frame):
    def __init__(self, master=None):
        super().__init__(master)
        self.pw = ttk.PanedWindow(orient=tk.HORIZONTAL,height=500)
        # self.instruction.pack()
        self.input_code = tk.Text(self.pw, height=10, width=50, wrap=tk.WORD)
        self.output_code = tk.Text(self.pw, height=10, width=50, wrap=tk.WORD)
        self.instruction = tk.Text(self, height=10, width=100)
        # self.pw.add(self.output_code)
        self.wrap(None)
        self.pw.grid(row=0, column=0, columnspan=2)
        self.instruction.grid(row=1, column=3, columnspan=2)
        # self.pw.pack()
